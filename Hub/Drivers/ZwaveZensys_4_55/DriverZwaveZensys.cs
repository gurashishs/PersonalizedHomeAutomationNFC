...
namespace HomeOS.Hub.Drivers.ZwaveZensys
{
    ...
    public class DriverZwaveZensys : Common.ModuleBase
    {
        ...
        public override void Start()
        {
            ...
            {
                logger.Log("ERROR: cannot find the zwave definitions file at " + fullPathDefinitionFile);
                return;
            }
        ...
        private void PrintConfiguration()
        {
            ...
            {
                logger.Log("device = {0}", device.Name);
        ...
        private bool FindController()
        {
            ...
            foreach (Zensys.Framework.Win32PnPEntityClass serialPortInfo in interfaces)
            {
                ...
                {
                    logger.Log("skipping {0} while search for zwave controller", serialPortInfo.Caption);
                    continue;
                }
        ...
        private void InitController()
        {
            ...
            foreach (IDeviceInfo nodeInfo in nodeList)
            {
                ...
                if (nodeInfo.Generic == XmlDataManager.FindGenericDevice("GENERIC_TYPE_STATIC_CONTROLLER").KeyId)
                {
                    ...
                    logger.Log("Setting Zwave controller Id to " + nodeInfo.Id);
                    continue;
                }
    ...
    //while (true)
        ...
        private string RemoveFailedNode(byte nodeId)
        {
            ...
            while (numRemainingTries > 0)
            {
                ...
                catch (Exception e)
                {
                    ...
                    logger.Log("Exception in removing {0}: {1}. NumRemainingTries = {2}", nodeId.ToString(), e.Message, numRemainingTries.ToString());
                }
        ...
        private string AddDevice(string deviceType)
        {
            ...
            while (maxtries > 0)
            {
                ...
                {
                    logger.Log("Got exception while finding device: " + e.ToString());
                }
        ...
        private void InitDevice(IDeviceInfo deviceInfo)
        {
            if (!zwaveNodes.ContainsKey(deviceInfo.Id))
            {
                ...
                if (details == null) 
                {
                    logger.Log("Details not found in memory for device {0}", deviceInfo.Id.ToString());
        ...
        private void ConfigureDevice(IDeviceInfo deviceInfo, string deviceType)
        {
            ...
            if (deviceConfiguration != null)
            {
                foreach (DeviceSettingElement setting in deviceConfiguration.DeviceSettings)
                {
                    ...
                    if (byteVals.Length != setting.Level)
                        logger.Log("zwave device configuration seems broken!");
                    else
            ...
            else
            {
                if (deviceType != "unknown")
                    logger.Log("zwave device of type {0} not found in configuration", deviceType);
            }
        }
        ...
        {
            logger.Log("Zwave detected USB insertion. Current value of foundController = {0}", foundController.ToString());
